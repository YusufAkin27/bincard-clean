...
public class MailService {
    ...
    // Kuyruğa e-posta ekle ve asenkron işlemi başlat
    public void queueEmail(EmailMessage emailMessage) {
        emailQueue.enqueue(emailMessage);
        processEmailAsync(emailMessage); // async çağrı
    }

    // Tek bir e-postayı asenkron gönder
    @Async("emailTaskExecutor")
    public void processEmailAsync(EmailMessage email) {
        sendEmail(email);
    }
    ...
    // Her 1 dakikada bir kuyruktaki e-postaları topluca gönder
    ...
    public void sendQueuedEmails() {
        ...
        if (!emailBatch.isEmpty()) {
            processBatchEmailsAsync(emailBatch); // async çağrı
        }
    }

    // Toplu e-postaları asenkron gönder
    @Async("emailTaskExecutor")
    public void processBatchEmailsAsync(List<EmailMessage> emailBatch) {
